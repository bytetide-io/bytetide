...
export function BentoCard({
    ...
    return (
        <motion.div
            initial="idle" // Set the initial animation state
            animate="active" // Keep the animation running in the "active" state
            variants={{
                idle: { opacity: 1, scale: 1 }, // Define idle state
                active: {
                    // scale: [1, 1.05, 1], // Example animation: scaling effect
                    opacity: [1, 0.95, 1], // Example animation: fading effect
                    transition: {
                        duration: 2, // Set the duration for one loop
